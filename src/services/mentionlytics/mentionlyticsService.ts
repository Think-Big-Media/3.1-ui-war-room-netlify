...
class MentionlyticsService {
  ...
  constructor() {
    ...
    // Check if we're in mock mode - fix the logic to handle both string and boolean values
    const envUseMock = import.meta.env.VITE_USE_MOCK_DATA;
    const localStorageUseMock = localStorage.getItem('VITE_USE_MOCK_DATA');
    
    // Convert to boolean - handle string 'true'/'false' and actual booleans
    const forceMock = envUseMock === 'true' || envUseMock === true || 
                      localStorageUseMock === 'true' ||
                      this.config.features.mockMode;
    
    // Use mock mode if explicitly forced OR if no API token available
    this.isMockMode = forceMock || !this.config.mentionlytics.apiToken;
  }
